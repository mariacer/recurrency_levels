...
#
# @title          :real_world_benchmarks/run_audioset.py
# @author         :mc
...
# python_version  :3.7
"""
Main script from which to run Audioset experiments
--------------------------------------------------
"""
...
def run(config=None):
    ...
    ### Setup.
    if config is None:
        config = parse_cmd_arguments(experiment='audioset')
    device, writer, logger = sutils.setup_environment(config,
        script_name=globals()['__file__'])   
    logger.info('Running Audioset experiment.')
