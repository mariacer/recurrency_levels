...
The result of this script is available at
    ...
    https://www.dropbox.com/s/yrpk7568a3kar0h/embeddings.pickle?dl=0

If you want to recreate or modify this dataset, download the data from
...
The corresponding folders should be located in:
    ...
    ``datasets/mud/universal_dependencies``
...
and the corresponding pickle files for each language should be located in:
    ...
    ``datasets/mud/word_embeddings``

The implementation is based on an implementation by Maria Cervera in
another project:
...
def preprocess_language(data, words, tagset, max_num_tokens=-1, 
    ...
    x = -np.ones((num_sentences, max_sentence_len))
    y = -np.ones((num_sentences, max_sentence_len))
...
if __name__=='__main__':
    ...
    download_dir = 'datasets/mud'
    target_path = 'datasets/mud'
    ...
    for l in range(num_languages):
        ...
        ### Get 17 tags by looking at all tagged train sequences.
